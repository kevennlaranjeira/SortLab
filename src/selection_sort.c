...
void selection_sort(int* vetor, int tamanho) {
    for (int i = 0; i < tamanho - 1; i++) {
        int min_index = i;  // Assume o índice atual como o menor
        for (int j = i + 1; j < tamanho; j++) {
            if (vetor[j] < vetor[min_index]) {  // Verifica se encontrou um menor valor
                min_index = j;
            }
        }
...
void executar_selection_sort() {
    ...
    char pasta_principal[] = "SelectionSort";
    ...
    // Arquivos de entrada, saída e tempo
    ...
    gerar_arquivo_entrada(nome_entrada, tamanho, tipo);  // Gera arquivo com os dados

    // Alocação e carregamento do vetor
    ...
    selection_sort(vetor, tamanho);  // Ordena o vetor
